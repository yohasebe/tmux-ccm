...
### Fixed
- Projects could stay stuck in BUSY indefinitely after a turn completed without a Stop hook firing. `hook_busy_idle` now only trusts a BUSY hook when the project's JSONL session log is also being updated (i.e. real tool / message activity is happening). If BOTH the hook AND the JSONL have been silent for longer than `BUSY_HOOK_JSONL_WINDOW` (default 600s, configurable via `CCM_BUSY_HOOK_JSONL_WINDOW`), the rule releases and the state falls through to the fallback path so it can eventually leave BUSY. A sibling rule `hook_busy_idle_no_jsonl` preserves the old indefinite-trust behavior for the edge case where a project has no JSONL file at all (older Claude Code, tests). Adds a `jsonl_missing` Rule axis to express this distinction cleanly
...
### Added
- **`@ccm_shell_history` reset on attach.** When the user attaches to a project (via dashboard, tree view, or `ccm attach`), the `@ccm_shell_history` cluster history is unset alongside `@ccm_done` and `@ccm_prev_state`. The user attaching is treated as acknowledgement of the canary — the SHELL cluster warning disappears immediately and only reappears if NEW transitions cluster after the attach. `cmd_send --start` does NOT clear the history because it is a remote operation, not a user acknowledgement
- **Cluster-SHELL-transition canary for anthropics/claude-code#48069.** Each time a project transitions into SHELL state from a non-SHELL predecessor, ccm records a timestamp in the per-window tmux option `@ccm_shell_history`. When `SHELL_CLUSTER_COUNT` (default 3) transitions accumulate within `SHELL_CLUSTER_WINDOW` (default 600s), `ccm status` and the dashboard surface a per-project warning pointing at the upstream silent-exit regression and noting that the conversation auto-restores via `claude --continue`. Thresholds are tunable via `CCM_SHELL_CLUSTER_COUNT` / `CCM_SHELL_CLUSTER_WINDOW`. The history entries older than the window are filtered on read, and each write caps the stored value at `2 × SHELL_CLUSTER_COUNT` entries (floor 6) to prevent unbounded growth. Designed to help users on macOS Claude Code v2.1.107+ diagnose repeated session drops without blaming ccm
- **`ccm send` — cross-project prompt injection.** Send a prompt to another project's Claude Code session without hand-crafting `tmux send-keys -l ... Enter`. Supports positional message, `--file <path>`, `--stdin`, or a `-` stdin alias. Multi-line messages are converted to `M-Enter` between lines and a final `Enter`, so the body arrives as a single multi-line prompt rather than multiple submitted turns. State-gated by default: IDLE and DONE accept sends, BUSY requires `--force` (message queues into the input buffer), SHELL requires `--start` (launches Claude first then waits 2s), and **PERMIT is a hard guard — refused even with `--force`** to prevent accidental permission-dialog approvals. `--no-enter` suppresses the final submit, `-y` / `--yes` skips the confirmation prompt, and `--` ends flag parsing for messages that start with `-`. Confirmation is auto-skipped when stdin or stdout is not a TTY, so `echo "msg" | ccm send blog --stdin` works inline from MCP servers or shell pipes. Defensive `send-keys -X cancel` is issued on the target pane first to protect against the copy-mode stuck-pane class of bug
- **Live incremental filter search in the dashboard** (rewrite of the `/` action). Typing a query filters the project list on every keystroke (case-insensitive substring match), Up/Down navigates the filtered candidates, Enter attaches directly to the selected project, and Esc/Ctrl-C/Ctrl-G cancels. Unicode-safe: Japanese project names (e.g. `文脈解析エンジン`) can be filtered by Japanese substrings (`文脈`) because the match uses Python's codepoint-level `in` on lowercased strings. Backspace is grapheme-aware via the existing `_strip_last_grapheme` helper so combining marks and CJK are deleted as single user-perceived units. Shortcuts `[C-u]` to clear the buffer, `[C-p]` / `[C-n]` as aliases for Up/Down
- **`ccm search` shortcut and `@ccm-key-search` tmux binding** — open the dashboard directly in the live-filter search, skipping the intermediate `Dashboard → /` keystroke. `ccm dashboard` also accepts `--search` for the same effect. Opt-in via `set -g @ccm-key-search "/"` in `~/.tmux.conf` to bind `prefix + /`
