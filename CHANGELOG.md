...
### Changed
- Eliminated false BUSY during attach to SHELL windows. Attaching to a SHELL project auto-starts Claude (`claude --continue`); Claude then spawns MCP servers as direct children for 5–30 s before the `❯` prompt renders. `detect_pane_state` sees `has_child=True + no prompt` and returns `raw=BUSY` — identical to a streaming response, so the dashboard flashed BUSY for ~10 s after every attach. A new `startup_transient_raw_busy` detection rule distinguishes this authoritatively: if `prev_state ∈ ("", "SHELL")` AND the hook signal is empty (no `UserPromptSubmit` / `PreToolUse` has fired yet), the rule demotes raw=BUSY to IDLE with `action=HOLD_NO_WRITE` so prev_state stays `SHELL` across the startup window. Related: `reset_window_after_attach` no longer wipes `@ccm_prev_state` — the wipe made attached-to-BUSY sessions indistinguishable from startups. Only `@ccm_completed_at` (stale ✓ marker) and `@ccm_shell_history` (cluster canary) are still cleared on attach. A new `hook_missing` condition axis on `Rule` supports the "no hook signal present" check
