...
> [!IMPORTANT]
> **Upgrade note**: Claude Code **v2.1.107 or later is required**. `ccm setup-hooks` hard-fails on older versions instead of silently registering an incomplete hook set. Run `claude update` first if you are on an earlier release.
...
- **`ccm clear-notifications`** — bulk-removes ccm notifications from macOS Notification Center. Requires `terminal-notifier`. Scoped to `ccm-`-prefixed group ids only, so notifications from other terminal-notifier-using scripts are left intact.
...
- **Themable mode-2 colours** via `@ccm-status-bg` / `@ccm-status-gutter-bg` / `@ccm-status-fg` / `@ccm-status-fg-dim` tmux options. Defaults match the dark-grey palette; override any subset to integrate with light themes or custom dark schemes without forking ccm. Invalid values silently fall back to the default.
...
### Changed
- **Claude Code v2.1.107+ is now required (BREAKING).** `ccm setup-hooks` hard-fails on older clients (or when `claude` is not on PATH) instead of silently skipping the `elicitation_dialog` Notification matcher. The matcher is registered unconditionally; `ccm_hooks_configured` always requires it.
- **Slow-path detection: ~7× fewer tmux subprocess calls per refresh.** `ccm status` / dashboard refresh / `ccm tree` previously fork-exec'd `tmux` 44× on a 13-window session (one `show-option @ccm_shell_history` per window in `shell_cluster_warnings`, one unconditional `set-option -wt @ccm_prev_state` per window in `apply_actions`, one unconditional `set-option -wut @ccm_bg_active` per window). Now: `shell_cluster_warnings` reads every window's history in a single `list-windows -F` (13 → 1), and the per-window writes both elide when the value is already correct (which is the steady state). End-to-end `ccm status` dropped from ~1.93 s to ~0.22 s on the same session.
...
- **`ccm init` aborted silently when `setup-hooks` failed.** The wizard called `ccm_setup_hooks 2>/dev/null`, so a hard-fail on too-old Claude Code (or `claude` not on PATH) killed the entire init script with no message. Now run in a subshell with stderr surfaced; setup failure logs a visible message and the wizard continues to subsequent steps.
- **`* elapsed` marker rendered on BUSY/PERMIT rows in the dashboard.** `@ccm_completed_at` is written on BUSY/PERMIT → IDLE transitions and not cleared on subsequent transitions, so a project that bounced IDLE → BUSY within `COMPLETED_AT_TIMEOUT` (30 s default) rendered as `◉ BUSY * 5s` — semantically nonsense, since the marker means "finished 5 s ago" but the project is busy now. Dashboard now suppresses the marker unless `proj.state == "IDLE"`. Mode 1 / 2 status bars never rendered the marker, so they were already correct.
