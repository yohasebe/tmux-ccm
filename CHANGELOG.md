...
### Changed
- **Minimum required Claude Code version raised from v2.1.101 to v2.1.107.** ccm now registers the `elicitation_dialog` Notification matcher introduced in v2.1.107. v2.1.101–v2.1.106 may not gracefully accept an unknown matcher value within a known event (the v2.1.101 "settings resilience" fix only confirmed safe handling for unknown event *names*, not unknown matcher *values*), and could drop the entire hook section. README and documentation now state v2.1.107 as the minimum supported version
