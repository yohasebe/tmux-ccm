...
- Cwd-drift handling. Claude Code hooks key signal/events files on the session's *current* cwd. If the user `cd`s into a subdirectory mid-conversation (`cd docker/services/ruby` etc.), the hook key diverges from `md5(@ccm_dir)` and the project would silently appear stuck (no fresh signals at the registered path). Each hook now writes a `<KEY>.cwd` sidecar with the literal cwd; `_resolve_hook_key` falls back to scanning sidecars for a descendant of `@ccm_dir` when the primary key has no signal. Fast path stays O(1); fallback scan caches per project. `cleanup_project_runtime_files` sweeps drifted keys via the same index so unregistering leaves no orphan files behind.
