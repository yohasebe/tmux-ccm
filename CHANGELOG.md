...
### Added
- **`ccm search` shortcut and `@ccm-key-search` tmux binding** — open the dashboard directly in quick-search mode (type, Enter to jump to the first matching project), skipping the intermediate `Dashboard → /` keystroke. `ccm dashboard` also accepts `--search` for the same effect. Opt-in via `set -g @ccm-key-search "/"` in `~/.tmux.conf` to bind `prefix + /`. Note that the dashboard's search is a prompt-based jump, not a live incremental filter
