...
| 値 | モード | 説明 |
|----|--------|------|
| `0` | 全詳細 | status-rightにアクティブプロジェクト名+アイコン一覧を表示 |
| `1` | アイコン（デフォルト） | status-rightにアイコン1つを追記 |
| `2` | 専用行 | IDLEを含む全プロジェクトを専用ステータス行に表示 |

#### モード0 — 全詳細をstatus-rightに表示

アクティブ（BUSY/PERMIT/DONE）プロジェクトの名前+アイコンをstatus-rightに表示。全てIDLE時は `≡` を表示。情報量は最大だが、既存のstatus-right内容を上書きする。
