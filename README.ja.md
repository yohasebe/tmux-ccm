...
| 値 | モード | 説明 |
|----|--------|------|
| `0` | 最小 | ウィンドウ名のアイコンのみ（status-right変更なし） |
| `1` | アイコン（デフォルト） | status-rightにアイコン1つを追記 |
| `2` | 専用行 | ブランチ・ポート情報付きで全プロジェクトを専用行に表示 |

#### モード0 — 最小表示（ウィンドウアイコンのみ）

status-rightは一切変更しない。tmux標準のウィンドウリストにアイコン（`⚠ project`、`◉ project` 等）が動的に付与されるのみ。tmuxのレイアウトを完全に維持する最も軽量なオプション。
