...
[![tested](https://img.shields.io/badge/tested-Claude%20Code%20v2.1.273-brightgreen)](https://docs.anthropic.com/en/docs/claude-code)
...
モード 1 は tmux のウィンドウリストを丸ごと置き換えるため、ccm が管理していない
ウィンドウはリストから消えます。同じセッションで通常の tmux 作業も行う場合は、
ウィンドウリストに触れないモード 0 か 2 を選んでください。

#### 稼働中のセッションだけを表示する

登録プロジェクトが多いと大半が常に `SHELL` で、対応が必要な 1 件が埋もれます。
モード 1 と 2 は、Claude セッションが動いているウィンドウだけを表示できます:

```tmux
set -g @ccm-status-line-hide-shell on
```

`IDLE` は表示されたままです — セッションは生きていて入力を待っており、ターンが
終わったときに `* elapsed` マーカーが出るのもこの状態です。隠れるのは `SHELL`
だけです。なお、アイドルセッションは `CCM_IDLE_EXIT_TIMEOUT` 後に自動終了するため、
放置したプロジェクトはセッション終了とともにバーから消えます（アタッチして Claude が
再起動すれば戻ります）。ダッシュボードと `ccm status` は常に全プロジェクトを表示します。

#### モード1のエントリを左側に置く

モード1は `status-right` に書き込み、tmux はこれを右寄せで描画するため、
エントリは時計の隣に並び、最優先が右端になります（幅が尽きたときに残る側です）。
左から右へ読む並びにするには:
...
エントリがバーの左側へ移動し、最優先が先頭になります。ウィンドウリストを消した
ことで空いた中央も埋まります。`status-left` には一切書き込みません — 押し出しに
使う余白は `status-right` 内にあるので、テーマの左側はそのままです。バーが狭くて
テーマの左側に潜り込んでしまう場合は、最も見たいエントリが切れるのを避けるため、
自動的に右寄せの配置を維持します。
...
| `n` | 選択プロジェクトの名前変更 |
| `g` | 既存ウィンドウを登録 |
| `r` | 削除 — [u]nregister（ウィンドウ残す）か [d]elete（閉じる。y/N 確認あり）を選択。メニューにも項目あり |
| `i` | 無視トグル — プロジェクトを隠す/戻す（[別モデルをサイドキックとして使う](docs/guide.ja.md#別モデルをサイドキックとして使うccm_ignore)参照） |
| `x` | アイドル状態のClaude Codeを一括終了 |
| `/` | プロジェクト名で検索 |
| `t` | ツリービューに切替 |
| `w` | サイドキックの注意喚起を切替 — あなたを待っているサイドキックを強調表示（[サイドキックの注意喚起](docs/guide.ja.md#サイドキックの注意喚起-いつあなたを必要としているかを知る)参照） |
...
ccm spool list|cancel|clear-expired|clear-held  キュー済み（store-and-forward）メッセージの確認 / 取り下げ / 未達・保持中の了解
...
SHELL状態（Claude Codeが終了済み）のプロジェクトウィンドウに切り替えると、ccmが自動的に `--continue` 付きでClaude Codeを再起動し、会話を再開します（既にあるシェルへは常に `--continue`。素の `claude` を打つのは `ccm add` だけで、それもそのディレクトリにまだ会話が無いと ccm が確認できたときに限ります。`--continue` の後ろにフォールバックは繋がないので、再開に失敗したときは黙って新しい会話を始めるのではなく、その理由が画面に残ります）。コマンドを打つ直前にウィンドウを読み直し、その時点で既にClaude Codeを載せたペインがある場合（アップデートによるその場での再起動など）や、シェルプロンプトと確認できるペインが無い場合は何も打ちません。
