...
マシンがスリープしたあとや、クライアントをデタッチして再アタッチしたあとに、ウィンドウ内のペインが入れ替わって戻ってくることがあります。左に置いていたペインが右に来ている、という形です。ccm 自体は影響を受けません。ペインは ID で指定し、Claude はプロセスツリーを辿って見つけるので、検出も `ccm send` も位置に依存しないためです。変わるのはあなたが作った配置だけです。

ccm はこの入れ替わりを記録するので、実際に起きているか・どのくらいの頻度かを確認できます:

```bash
ccm doctor          # "pane reorders  N in last 24h"
```

各イベントは `~/.local/share/ccm/state/pane-order.log` に 1 行ずつ、入れ替わり前後の並び付きで記録されます (1 プロジェクトあたり毎分 1 件にレート制限)。

ccm はペインを元に戻しません。入れ替わりと、あなたが意図的に並べ替えた場合は外から見て区別できないため、自動で復元するとあなた自身が動かしたときに邪魔をすることになります。ペインの増減 (tmux サーバー再起動で新しいペイン ID が振られる場合を含む) は入れ替わりとして記録されません。同じペインが別の順序になった場合だけです。

配置を直すときは自分で入れ替えてください:
...
- **Claude → 自分のウィンドウのサイドキック**: まず `ccm capture <自分のプロジェクト>` で相手が入力可能か確認し（ccm は Claude 以外のペインを state-gate できません）、本文と送信キーを分けて送ります:
  ```bash
  tmux send-keys -t <pane> -l -- "<message>"   # -l: リテラル送信（キー名として解決しない）
  sleep 0.3                                    # composer が落ち着くのを待つ（下記参照）
  tmux send-keys -t <pane> Enter
  ```
  `-l` は必須です。付けないと tmux は引数を**キー名**として解決するため、`Space` や `Enter` といった語を含むメッセージがそのキー入力に化けます（ccm 自身の配送も同じ理由で `-l --` を使っています）。改行キーは CLI ごとに異なるため（Claude は `M-Enter`、他の多くは `C-j`）、複数行の本文は各行をリテラル送信し、間に相手の改行キーを挟みます。

  **`Enter` の前の待ちは必須で、ここが最も踏みやすい失敗です。** 2 つの `send-keys` を `&&` で連結すると、挿入されたテキストを相手の TUI がまだ処理している最中に `Enter` が届き、submit ではなく**改行**として解釈されることがあります。本文は composer に残ったまま、送信できたときとまったく同じ見た目になります。Kimi K3 での実測では、間隔なしは毎回失敗し、0.3 秒と 1 秒はどちらも送信されました。Claude Code の composer は間隔なしでも耐えるため、`ccm send` に待ちは不要で、相手が Claude 以外のときだけこの問題が出ます。

  **したがって、送信できたことを確認してください。推測しないこと。** `Enter` の後に `ccm capture` して相手の入力欄を見ます。**空なら送信済み**、**自分のテキストが残っていれば未送信**です。見えているテキストを配送の証拠と読むのは逆で、それは未送信の証拠です。`Enter` の前に確認できればさらに良く、`ccm send` は最も危険な経路でそうしています（`lib/ccm_send.py`）: 本文が着弾したか確認し、駄目なら打ち直し、そのうえで `Enter` を送る。この順序なら再試行が二重送信になりません。
- **サイドキックは自分のウィンドウにだけ従う。** `ccm send` が届くのはプロジェクトの **Claude** であって、サイドキックには決して届きません（横取りされないよう配送候補から除外されています）。生の `tmux send-keys` も同じ境界を守ってください。打ち込んでよいのは自分のウィンドウを共有しているサイドキックだけで、他プロジェクトのサイドキックに用があるときは、そのプロジェクトの Claude に頼んで中継してもらいます。相手が空いているかも、その TUI がどのキーを受けるかも、知っているのはその Claude であって、外から見るあなたは両方とも推測になります。さらにその Claude は自分のサイドキックが今何をしているかを把握し続けています。外から割り込むと、2 人の送信者が 1 つの composer に着弾し、混ざって 1 つの壊れたプロンプトになります。これは ccm が強制するものではなく規約です — tmux はあなたのものであって ccm のものではありません。
