...
新しいtmuxウィンドウが作られ、プロジェクトディレクトリに移動して、Claude Codeが起動します — `claude --continue`（そのディレクトリの最新の会話を再開）、またはそのディレクトリにまだ会話が無いと ccm が確認できたときだけ素の `claude` です。確認できたと言えるのは、CLI の設定ホームが既定で、トランスクリプトディレクトリ名のカスタム指定（`CLAUDE_CODE_PROJECT_DIR_NAME`）が ccm の環境にも新しいウィンドウのシェルが継承する tmux の環境にも無く、そのパスに対応するトランスクリプトディレクトリが存在しないか空のときです。ccm が確認できないものはすべて `--continue` になります。確認の範囲は ccm と tmux の環境に見えるものまでで、シェルの起動ファイルがトランスクリプトの保存先を変えている場合は対象外です。その場合、会話が別の保存先にあるディレクトリで `ccm add` が新規会話を開くことがあります。
...
> [!TIP]
> Claude Codeが動いていないウィンドウに切り替えると、`claude --continue` で自動的に前回の会話を再開します。既にあるシェルへの起動は常に `--continue` です — そのシェルは ccm から見えない場所（シェル自身の export は外からは見えません）にトランスクリプトを置いているかもしれないので、ccm は「再開するものが無い」とは決して主張しません。素の `claude` を打つのは、ccm 自身がウィンドウを作る `ccm add` で、そのディレクトリにまだ会話が無いときだけです。`--continue` の後ろにフォールバックは繋いでいません。再開できないとき（結局再開するものが無かった場合 — auto-start を切って追加したプロジェクトなど — や、最新の会話が生きたバックグラウンドセッションに引き継がれていた場合）は、そのメッセージが画面に残りペインはシェルに戻るので、次に何を開くかはあなたが決めます。
...
```bash
ccm spool list                    # 配送待ち・保持・期限切れの一覧（本文冒頭付き）
ccm spool clear-expired           # 未達のまま期限切れになった記録を了解済みにする
ccm spool clear-held [project]    # 相手が保持したままの記録を了解済みにする
ccm spool cancel <id> <project>   # 1件取り下げ（キュー済みの誤送信は取り消せます）
ccm spool cancel --all <project>  # そのプロジェクトのキューを全消去
```

期限切れのメッセージは未配達です。`ccm spool list [project]` は各記録の id、保存された送信元ラベル、キューに入ってからの経過時間、最初の空でない行（60文字まで）を表示します。規約外のファイル名は送信元・時刻を unknown とし、本文が読めない場合も `(unreadable)` として記録を表示します。各期限切れ記録には全文を読むコマンドと、**今も必要な依頼か確認した場合にだけ使う** `ccm send --file …` コマンドを添えます。修正が必要なら更新した本文を送ってください。表示されたコマンドが自動実行されることはありません。

送り直す操作は**新しい送信**です。キューされる場合は新しい id と送信時刻になり、配達時の envelope には現在の送信者が記載されます。受信可能な相手には通常の send と同じ経路で直接送信します。SHELL の相手にはキューされ、提示するコマンドは `--start` を使いません。元の期限切れ記録は通常の7日間の保存期限または了解操作まで残ります。プロジェクトの**全期限切れ記録**を確認した後、`ccm spool clear-expired <project>` でその記録だけを削除できます。送信は行いません。プロジェクト省略時は全プロジェクトの期限切れ記録を消します。一覧表示も記録の削除も配送を再試行しません。

送信先のセッションが「受け取っていない」と表示したメッセージ（Claude Code は書き換えたプロンプトを利用者が確認するまで入力欄に保持し、その旨を入力欄の上に表示します）は、キューに戻しません。相手がそこで Enter を押しても消しても入力欄は同じ見た目になり、区別できないため、再送すると本文をもう一度打ち込むことになるからです。これは「保持された」として記録され、`ccm spool list` に表示されます。対処はそのセッションの画面で行い、済んだら `ccm spool clear-held` で記録を了解済みにします。このコマンドは送信も取り下げも行いません。
...
| `--start` | SHELL 状態なら Claude を自動起動してから送信。起動の直前に窓を読み直し、既に Claude を載せたペインがあれば何も打たずそこへ配送し、シェルプロンプトと確認できるペインが無ければ理由を示して送信を拒否する |
...
### どのペインが最後に返事をしたか（未読の印）

複数のエージェントをペインに並べていると、どれが最後に返事をしたのか分からなくなります。次で有効にします。

```bash
set -g @ccm-pane-labels on     # ~/.tmux.conf の、ccm を読み込むより前に書く
```

有効にすると、あなたが別の場所を見ている間に返事が完了したペインの枠に `◆ new` が付き、そのペインを数秒見続けるか、次のプロンプトを送ると消えます。この数秒は `@ccm-unread-linger`（既定 `5`）です。tmux は、ウィンドウを切り替えたりターミナルを前面に戻したりした瞬間に、そのペインへフォーカスを通知します。それは見に行く操作そのものなので、その通知で消すと、見る前に印が消えてしまいます。通り過ぎただけのペインの印は残ります。「見ている」とは、フォーカスのあるターミナルがそのペインをアクティブなペインとして表示している状態です。フォーカスは `set -g focus-events on` のときだけ tmux が報告します。ccm のダッシュボードのポップアップが開いている間は、その下のペインは見ているものとして扱いません。他のツールのポップアップは ccm から分からないので、その下で完了した返事には印が付きません。ccm にとって「読んだ」の代わりになるのは、その時間だけ保たれたフォーカスです。

印は完了通知と同じ条件で付きます。返事が `CCM_COMPLETION_GRACE_SEC` の間完了したままのときに付き（複数ステップの途中にあるツール境界は数えません）、バックグラウンドのタスクが残っている間は付きません。フックが必要なので、対象は Claude Code のセッションです。フックを持たないサイドキックには付きません。

この設定は tmux のペイン枠（`pane-border-status top`）を有効にし、`pane-border-format` の先頭に印を足します。ご自分の設定で ccm の読み込みより後に `pane-border-format` を設定している場合は、そちらが優先されるので、次をご自分の書式に足してください。

```
#{?@ccm_unread,#[fg=colour209]◆ new#[default] ,}
```

オプションをオフにすると新しい印はすぐ付かなくなりますが、オンの間に設定されたもの（ペイン枠、書式、フォーカス時のフック）は、tmux を再起動するかご自分で戻すまで残ります。
...
### `claude --continue` が新規セッションになるとき

`/bg` でセッションをバックグラウンドに送ると、そのトランスクリプトの末尾に引き継ぎ（hand-off）レコードが書かれます。`claude --continue` はディレクトリのトランスクリプトを新しい順に辿り、最新のものが CLI がまだ生きていると判断するセッションへ引き継いでいる場合、何も復元しません — 通知でバックグラウンドセッションを名指しして、新規セッションが始まります。タスクが `done` になった後も、CLI はワーカーを「生きている」と判断することがあります。そのため ccm が attach 時に打つ起動コマンドが、会話はディスクに残っているのに空のセッションを開くことがあります（`claude --resume` のピッカーも引き継ぎ済みトランスクリプトを隠しますが、`claude --resume <完全なセッション id>` なら開けます）。

ccm は 3 つを安い順に確かめます。daemon の roster にそのプロジェクトの worker が載っていること（必要条件であって十分条件ではありません — roster は daemon より長生きするので、クラッシュや再起動の後も全 worker が死んだ pid のまま載っています）、最新トランスクリプトの末尾がその worker のセッションへ引き継いでいること、そして CLI 自身のセッション登録簿（`~/.claude/sessions/<pid>.json`、動作中プロセスごとに 1 レコード — `claude --continue` が参照する一覧）に、そのセッションの生きた非 interactive レコードがあることです。「生きている」は CLI 自身の基準で判定します: プロセスが存在し、起動時刻がレコードの `procStart` と一致すること。CLI が集合を確定しない状況（読めないレコード、kind や session id を欠く生きたレコード）では `claude --continue` は引き継ぎを素通りするので通知も出しません。ccm が CLI と同じ検証をできない場合（起動時刻が読めない、別の pid domain のレコード）や、`CLAUDE_CONFIG_DIR` で CLI のホームが `~/.claude`（ccm がトランスクリプトと登録簿を読む場所）から移されている場合も出しません。通知は起動が起きる場所・起きた直後に出ます:

- ダッシュボードと `ccm status` の `⚠` 行（プロジェクト名、バックグラウンドセッション、その状態、出口）— SHELL 状態のプロジェクトに限ります。既に Claude が動いているウィンドウ（多くはそのバックグラウンドセッション自身を attach したもの）には起動が来ないためです
- `ccm doctor` の `bg hand-off` 行
- `ccm attach` やダッシュボードからの attach が起動コマンドを打った直後の tmux メッセージ行、および `ccm attach` / `ccm send --start` の出力

出口は CLI 自身のものです: `claude attach <short>` はバックグラウンド側の会話（引き継ぎ後の作業が入っている方）を開き、`claude stop <short>` は引き継ぎを解放して `--continue` / `--resume` が先へ進めるようにし、`claude rm <short>` は完了済みセッションを roster から外します。この通知はどれも代行しません — 検査は読み取り専用です（ダッシュボードの bg 行で `Enter` を押すと `claude attach` が打たれますが、それは別の明示的な操作です）。ディレクトリに新しい対話セッションができれば、次の更新で通知は消えます。
...
**自動終了は打つ前に画面を見ます。** IDLE は「会話が止まっている」ことを言うだけで、「その画面にスラッシュコマンドを打ってよい」ことは言いません。バックグラウンドに送られたセッション（`/background`、空プロンプトで `←` 2 回、attach 中のバックグラウンドセッションでの `/exit` — これは終了ではなく detach です）はペインを agent view のまま残します。そこでは入力欄に打たれた文字が新しいセッションを始めることがあり、Escape は入力の取り消しではなく画面の切り替えです。そこで auto-exit は対象ペインを先に capture し、agent view が表示されているか読めないときは何も打ちません。また `/exit` の後にペインが agent view になった場合 — attach 中のバックグラウンドセッションに exit を指示したときの挙動で、detach して worker は動き続けます — は終了として扱わず、SHELL 状態の書き込み・autosave・完了通知・追加のキー送信をいずれも行いません。観測しているのは画面だけなので、記録には「何が見えたか」を書き、原因は断定しません。どちらも `$CCM_DATA_DIR/state/auto-exit-declined.log` に記録します（結果種別と識別子のみ、プロジェクト × 結果種別ごとにレート制限）。`auto-exit.log` とは分けてあり、あちらは本当の終了だけを数え続けます。`ccm doctor` は両方の件数を出します。要するに attach 中のバックグラウンドセッションは auto-exit が終わらせるものではありません — worker はどのみち動き続け、止めるのは `claude stop <short>` の仕事です。
