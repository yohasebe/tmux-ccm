...
有効にすると、あなたが別の場所を見ている間に返事が完了したペインの枠に `◆ new` が付き、そのペインにフォーカスするか次のプロンプトを送ると消えます。「見ている」とは、フォーカスのあるターミナルがそのペインをアクティブなペインとして表示している状態です。フォーカスは `set -g focus-events on` のときだけ tmux が報告します。ccm のダッシュボードのポップアップが開いている間は、その下のペインは見ているものとして扱いません。他のツールのポップアップは ccm から分からないので、その下で完了した返事には印が付きません。ccm にとって「読んだ」の代わりになるのはフォーカスだけなので、通り過ぎただけでも印は消えます。
