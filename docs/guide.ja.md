...
| `u` | 未配送の連絡 | expired / held の一覧を開く。メニュー（`m` / `?`）からも開ける |
...
`u` で、期限切れで未配送の expired と、相手の入力欄に残っていた held の一覧を開けます。各記録に送信元・宛先・経過時間・本文の書き出しを表示します。

| キー | 一覧での操作 |
|------|--------------|
| `↑↓` / `jk` | 記録を選択 |
| `Enter` | 全文を表示。長い行は折り返し、`↑↓` / `jk` / `PgUp` / `PgDn` でスクロール。`q` / `Esc` / `Enter` で一覧へ戻る |
| `r` | expired を新しい送信として送り直す。確認が必要。Claude が停止中なら起動して送るか確認する。成功したときだけ元の記録を削除し、失敗したら理由を表示して記録を残す |
| `d` | 確認して選択した1件だけを削除。held の記録を消しても相手の入力欄は変更しない |
| `o` | 宛先の既存プロジェクトを開く。打鍵や Claude の自動起動はしない。入力欄の確認や手動起動に使う |
| `q` / `Esc` | ダッシュボードへ戻る |

held には再送操作を表示しません。相手の入力欄にある本文を重ねて送らないため、まず `o` で確認してください。再送・削除は既定 No で、確認をキャンセルすれば何もしません。一覧は表示時と操作後に読み込み、既存の順序と残っている選択対象を保ちます。新しい記録は末尾に追加されます。外部の変更を反映したい場合は一覧を開き直してください。
...
サイドキックや sandbox 内のツールから `ccm send` を呼ぶ場合は、tmux の Unix
ソケットに接続できるよう sandbox の外で実行してください。`TMUX` が設定されていても、
ネットワーク制限でソケットへの接続が遮断されることがあります。接続に失敗した場合は
元の tmux エラーと sandbox の外での再実行案内を表示します。プロジェクトがない、
あるいは呼び出し元が tmux の外にいる、という意味ではありません。

予約メッセージは、環境変数が指すペインのプロセスが送信コマンドの祖先にあれば、
そのペインの登録プロジェクトを送信元とします。別プロジェクトへディレクトリを移動しても、
送信元とペインの識別は維持されます。プロセス情報の取得失敗などで祖先関係を確認できない
場合は、そのペインの登録ディレクトリが送信コマンドの作業ディレクトリを含むか照合します。
両方のパスで symlink を解決し、サブディレクトリは一致としますが、名前の先頭が同じだけの
別ディレクトリは一致としません。不一致なら、作業ディレクトリを含む登録窓がちょうど
一つのときだけ補完します。入れ子や重複登録は曖昧とし、同じ tmux 窓をリンクしたものは
一つと数えます。決まらなければ送信元は `unknown` です。配達時のヘッダーは
`ccm send unknown` を勧めず、受信側に返信先の確認を求めます。

一意の登録窓からの補完で分かるのはプロジェクトで、呼び出し元の特定ペインではありません。
自己宛て判定と `--start` のペイン除外には確認できたペインだけを使います。
`sidekick-send` とプロジェクト名を省略した `ignore` / `unignore` は未確認のペイン情報を
拒否するので、登録プロジェクトのペインから実行してください。祖先関係を確認できない場合、
同じディレクトリに登録されたペイン同士で環境変数が古い場合までは区別できません。
...
| **SHELL**(Claude 未起動) | キューに保存 — Claude が起動して IDLE になってから配送 | 拒否 | — | Claude を起動し、IDLE と空の入力欄の準備を確認（`CCM_START_WAIT_SEC` 秒、デフォルト10秒を期限にポーリング）してから送信 |
...
`--start` は起動先ペインに空の入力欄が現れ、IDLE とその状態が1秒間続くのを待ってから本文を打ちます。待機期限までに準備ができなければ、本文と送信用 Enter は打たず終了コード1で失敗を報告します。起動コマンドがすぐ終了してシェルへ戻る場合も同様です。状態検出や画面取得の所要時間は、設定したポーリング期限を超えることがあります。

起動後は本文を一度だけ打ち、入力欄に全文が見える場合だけ送信用 Enter を送ります。一部しか見えない、何も見えない、入力欄を読めない場合は、消去・打ち直し・送信をせず終了コード1で停止します。相手の入力欄を確認し、残った文字があれば消してから、送り直すか判断してください。短い本文や `--no-enter` でも同じ確認を行います。長文が入力欄から一部はみ出して見えない場合も、確認のため停止します。空の入力欄と IDLE は、起動直後のアプリがすべての打鍵を受け付ける保証にはなりません。また、画面での確認はモデルの受領を証明するものではありません。

送信用 Enter の後も入力欄を確認します。保留通知が出る場合や、送った本文の先頭が確認中ずっと入力欄の先頭に残る場合は、`Sent` ではなく未送信として終了コード1を返します。Enter を自動で押し直しません。相手の入力欄を確認してから対処してください。これは画面に基づく確認なので、画面が読めない場合や、受理直後に同じ書き出しの下書きが入力された場合などを完全に区別できるものではありません。
...
一覧に表示された `ccm send --file …` で送り直す操作は**新しい送信**です。キューされる場合は新しい id と送信時刻になり、配達時の envelope には現在の送信者が記載されます。受信可能な相手には通常の send と同じ経路で直接送信します。SHELL の相手にはキューされ、提示するコマンドは `--start` を使いません。元の期限切れ記録は通常の7日間の保存期限または了解操作まで残ります。プロジェクトの**全期限切れ記録**を確認した後、`ccm spool clear-expired <project>` でその記録だけを削除できます。送信は行いません。プロジェクト省略時は全プロジェクトの期限切れ記録を消します。一覧表示も記録の削除も配送を再試行しません。

1件を直接扱うには `show` で全文を読み、`discard` でその記録だけを削除できます。`resend expired` は保存した本文を**新しい送信**として今送り直し、送信確認に成功したときだけ元の記録を削除します。BUSY などで今送れない場合は新たにキューせず失敗し、元の記録を残します。Claude が停止中なら `--start` で起動してからの送信を指定できます。本文を修正したいときは別途 `ccm send` で更新した内容を送ってください。

`discard` と `resend` は確認を求めます。内容を確認済みなら `--yes`（`-y`）で確認を省略できます。非対話環境ではこの明示指定が必要です。held は `resend` で送り直せません。`discard held` は記録だけを消し、相手の入力欄には触れません。dashboard の `u` も同じ処理を使います。

送信先のセッションが「受け取っていない」と表示したメッセージ（Claude Code は書き換えたプロンプトを利用者が確認するまで入力欄に保持し、その旨を入力欄の上に表示します）は、キューに戻しません。相手がそこで Enter を押しても消しても入力欄は同じ見た目になり、区別できないため、再送すると本文をもう一度打ち込むことになるからです。保留通知がなくても送った本文の先頭が入力欄に残り続ける場合は同じ扱いです。これらは「保持された」として記録され、`ccm spool list` に表示されます。対処はそのセッションの画面で行い、済んだら `ccm spool clear-held` で記録を了解済みにします。このコマンドは送信も取り下げも行いません。
...
**忘れた場合。** 黙って壊れることはありません。可視の Claude セッションが2つあるウィンドウは両方を集約し、`ccm send` は誤ったほうの会話に打ち込む代わりに推測を拒否します — その拒否メッセージが `CCM_IGNORE` と `i` キーを案内します。`ccm doctor --verbose` も該当ウィンドウを *multi-claude windows* として列挙します。どちらも警告としては書かれていません。同じウィンドウ構成は Agent Teams の通常の分割でもあり、その場合はどちらの PERMIT も届くよう両方が可視のままである必要があるからです。両方の読みを示したうえで、判断はあなたに委ねられます。
...
以後、サイドキックが許可プロンプトに突き当たると、その hook が tmux ペインをキーにした attention marker を書き、ccm は全ての面で同時に反応します: `▸kimi` バッジがダッシュボード・`ccm status`・ステータスバーで PERMIT の黄色に変わり、何のツールについて尋ねているかを載せたデスクトップ通知が飛びます。承認ダイアログには、元のサイドキックのペインであなた自身が答えてください。Claude に代理承認をさせないでください。あなたがダイアログに答えると、解決側の hook が待ちを閉じ、バッジは dim に戻ります。ウィンドウ自体の状態は一切変わりません — PERMIT は今までどおり *Claude* があなたを必要としている印で、黄色い `▸` は*サイドキック*がそうである印です。
...
読むか破棄できます。自動通知は再送できません。`ccm doctor --verbose` は導入状態、受信の有無、窓ごとの
設定と結び付けも示します。受信は現在の信頼状態の証明ではありません。
証跡と重複識別用 ID は7日間保持します。

通知を有効にすると、抜粋は Claude の会話記録に入り、手元の外へ送られます。伏せ字は best-effort です。`tmux set -w @ccm-sidekick-notify-excerpt off` で抜粋を無効にできますが、Claude が発生元のペインを読む場合、その内容も会話記録に入り、手元の外へ送られます。
...
### 完了連絡の規約（`sidekick-completion v1`）

サイドキックに仕事を頼む依頼元が、完了連絡の経路（自動通知／明示送信）と結果の
置き場所を指定します。自動通知が指定されたら、完了を知らせるための `ccm send` を
追加せず、最終メッセージの先頭に「依頼番号・完了・結果の所在」を一行で書きます。
明示送信が指定されたら従来どおり指定の相手へ報告し、最終メッセージにも結果の所在を
残します。指示が不明なら依頼元に確かめ、推測で連絡を省かないでください。
承認ダイアログは常に利用者本人が元の UI で操作します。

自動通知だけでは、hook が動かない場合や、集約で古い抜粋が置き換わる場合に、結果の
所在が届かないことがあります。抜粋には伏せ字がかかり、400文字までに切られます。
抜粋 off では所在は含まれません。通知 off・未対応 CLI・経路に確信が持てない場合は
明示送信を指定してください。ccm に完了連絡の確認待ちを管理する機能はありません。
報告が見当たらなければサイドキックの結果を確認してください。expired / limited /
held / uncertain の通知を見て、自動で別経路から送り直してはいけません。
...
**自動終了は記録を残します。** Claude Code はこれを `SessionEnd` の reason `prompt_input_exit` として報告しますが、これは人間が `/exit` を打った場合とまったく同じ値なので、事後には両者を区別できません。そこで ccm は自動終了 1 回につき 1 行を `$CCM_DATA_DIR/state/auto-exit.log` に書きます（時刻・プロジェクト・session id・idle 秒）。件数は `ccm doctor --verbose` に出ます。「何かがセッションを閉じているのでは」と思ったときはこのファイルが答えになり、session id があるので、同じセッションの終了を観測していた別のツールの記録と突き合わせられます。

**自動終了は打つ前に画面を見ます。** IDLE は「会話が止まっている」ことを言うだけで、「その画面にスラッシュコマンドを打ってよい」ことは言いません。バックグラウンドに送られたセッション（`/background`、空プロンプトで `←` 2 回、attach 中のバックグラウンドセッションでの `/exit` — これは終了ではなく detach です）はペインを agent view のまま残します。そこでは入力欄に打たれた文字が新しいセッションを始めることがあり、Escape は入力の取り消しではなく画面の切り替えです。そこで auto-exit は対象ペインを先に capture し、agent view が表示されているか読めないときは何も打ちません。また `/exit` の後にペインが agent view になった場合 — attach 中のバックグラウンドセッションに exit を指示したときの挙動で、detach して worker は動き続けます — は終了として扱わず、SHELL 状態の書き込み・autosave・完了通知・追加のキー送信をいずれも行いません。観測しているのは画面だけなので、記録には「何が見えたか」を書き、原因は断定しません。どちらも `$CCM_DATA_DIR/state/auto-exit-declined.log` に記録します（結果種別と識別子のみ、プロジェクト × 結果種別ごとにレート制限）。`auto-exit.log` とは分けてあり、あちらは本当の終了だけを数え続けます。`ccm doctor --verbose` は両方の件数を出します。要するに attach 中のバックグラウンドセッションは auto-exit が終わらせるものではありません — worker はどのみち動き続け、止めるのは `claude stop <short>` の仕事です。
