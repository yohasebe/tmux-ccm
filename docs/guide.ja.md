...
| `--peer` | プロジェクト名を指定せず、**この**ウィンドウのもう一方の Claude ペインへ送信（`<name>` 不要）。主 + サイドキック構成向け。下記「別モデルをサイドキックとして使う」参照 |
...
## 2ペイン間のピア通信（CCM_IGNORE）

2つ目の Claude Code セッションを同じウィンドウの分割ペインで走らせ（主セッションの隣に相談用のサイドキックを置く）、2つのペイン間でメッセージを送り合えます。既定では ccm が両ペインを 1 つのウィンドウ状態に集約してしまい、`ccm send` がどちらに届くべきか判別できないため、`CCM_IGNORE` でサイドキックを ccm から不可視にし、主セッションだけをクリーンに追跡させます:
...
**2つのペイン間でメッセージを送り合う**には `ccm send --peer` を使います。どちらのペインからでも `ccm send --peer "..."` で同じウィンドウの*もう一方*の Claude ペインに届くので、プロジェクト名を指定せずに主セッションとサイドキックの間で（あるいはエージェント同士で）中継できます:

```bash
# 主ペイン → サイドキック
ccm send --peer "今説明した diff をレビューしてリスクを指摘して"

# サイドキック → 主ペイン
ccm send --peer "完了。リスクは fetch() のリトライループ"
```

もう一方の claude ホストペインを1つだけ対象にします（隣が素のシェルなら拒否、claude ペインが3つ以上で曖昧なら拒否）。`CCM_IGNORE` 済みのサイドキックは継続追跡していないため、そのペインの状態をその場で確認し、許可ダイアログ中には注入しません。これで「会話」チャネルは機能しつつ、2つのセッションは独立を保てます（コードは従来どおりファイルシステム/git で共有）。
