...
| `CCM_TRACE_ONLY_DIFF` | (unset) | When set to a truthy value (`1` / `true` / `yes` / `on`), restricts `CCM_DEBUG_TRACE` writes to rows where the legacy and event-log derivations disagree. Lets observe-mode runs span days on a busy multi-project tmux without hitting the size cap — the file only grows when a real diff appears. No effect when `CCM_USE_EVENT_LOG=off` (no event-log state to diff against) |
| `CCM_USE_EVENT_LOG` | `auto` | Selects the detection backbone. `auto` (default) commits the event-log state when [`derive_state_from_events`](../lib/ccm_detection.py) returns a non-`None` answer; otherwise the legacy `DETECTION_RULES` pipeline takes over. `off` (or `0` / `no` / `false`) restores pre-2026-04-25 legacy-only behaviour. `observe` runs both paths but commits legacy — pair with `CCM_DEBUG_TRACE` to surface diffs. `primary` (or `1` / `true` / `yes` / `on`) is reserved for diagnostic forced-mode use; today it dispatches identically to `auto`. Unknown values resolve to `auto` so a misspelled opt-out keeps you on the safer event-log path |
