#!/usr/bin/env bash
# ccm hook: Claude Code PreToolUse → write BUSY signal
# This solves the multi-turn gap: when Stop fires at a turn boundary,
# the next tool execution re-asserts BUSY via this hook.
# Installed by: ccm setup-hooks
# Input: JSON on stdin with { session_id, cwd, hook_event_name, tool_name, ... }
