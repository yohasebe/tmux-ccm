...
#
# Panes inside a ccm window sometimes come back in a different order
# after the machine sleeps or a client detaches and reattaches, while
# the tmux server itself stays up. ccm keeps working through it — panes
# are addressed by id and claude is found by walking the process tree,
# neither of which depends on position — so nothing in the state model
# notices. What breaks is the arrangement the user built: the pane they
# expect on the left is suddenly on the right.
#
# This is observation only. Nothing here moves a pane. Restoring order
# automatically would mean guessing whether a reorder was an accident
# or the user rearranging their own window, and there is no signal that
# separates the two; the cause of the drift is also still unknown. So
# ccm records that it happened and leaves the decision alone, the same
# observe-first posture the hook-silence canary starts from.
#
# Scope is deliberately "server still alive", which is what makes the
# check cheap and exact: pane ids are stable for the server's lifetime,
# so a pure reorder is literally the same id set in a different
# sequence. A server restart mints new ids, which reads as a different
# set and re-baselines silently rather than reporting a false drift.

PANE_ORDER_LOG_INTERVAL = int(
    os.environ.get("CCM_PANE_ORDER_LOG_INTERVAL", "60")
)
PANE_ORDER_LOG_MAX_BYTES = int(
    os.environ.get("CCM_PANE_ORDER_LOG_MAX_BYTES", str(1 * 1024 * 1024))
)


def pane_order_log_path() -> str:
    """Resolve the reorder-log path at call time so tests (and users)
    can redirect it via CCM_PANE_ORDER_LOG without reload tricks."""
    return os.environ.get(
        "CCM_PANE_ORDER_LOG",
        os.path.join(ccm_core.CCM_DATA_DIR, "state", "pane-order.log"),
    )


def classify_pane_order(previous: str, current: tuple) -> str:
    """Compare a window's remembered pane order against what it shows
    now. Pure — the caller owns every side effect.

    Returns one of:

    * ``"first"``    — nothing remembered yet; record a baseline
    * ``"same"``     — unchanged; nothing to do
    * ``"reorder"``  — same pane ids, different sequence. This is the
      event worth recording: no pane was opened or closed, they simply
      swapped places
    * ``"membership"`` — a pane appeared or disappeared. Ordinary
      editing (splitting, closing) and also what a tmux server restart
      looks like, since ids are minted fresh. Re-baseline, stay quiet

    Splitting `membership` out is what keeps the log honest. Treating
    any difference as a reorder would file every split and every server
    restart as evidence of the thing being investigated.
    """
    if not previous:
        return "first"
    before = tuple(p for p in previous.split(",") if p)
    if before == tuple(current):
        return "same"
...
def _log_pane_reorder(project_name, project_dir, before, after, now) -> None:
    """Append one JSON evidence record for a reorder, rate-limited per
    project. Best-effort throughout: an observer must never break the
    build it rides on. Rotation mirrors the hook-silence log."""
    try:
        log_path = pane_order_log_path()
        marker_dir = log_path + ".markers"
        os.makedirs(marker_dir, exist_ok=True)
        marker = os.path.join(
            marker_dir, ccm_core.md5_hash(project_dir or project_name))
        try:
            if now - os.path.getmtime(marker) < PANE_ORDER_LOG_INTERVAL:
                return
        except OSError:
            pass  # no marker yet → first reorder seen for this project

        try:
            if os.path.getsize(log_path) >= PANE_ORDER_LOG_MAX_BYTES:
                os.replace(log_path, log_path + ".1")
        except OSError:
            pass
...
def observe_pane_order(win_target, project_name, project_dir,
                       previous, current, now) -> str:
    """Record `current` as the window's pane order, logging evidence
    when it is a pure reorder. Returns the classification so callers
    (and tests) can see what was decided.

    The tmux write happens only when the order actually changed, so the
    steady state costs nothing beyond the comparison — the value itself
    rides the bulk `list-windows` query that every build already makes.
    """
    verdict = classify_pane_order(previous, current)
    if verdict == "same":
        return verdict
    if verdict == "reorder":
        before = tuple(p for p in previous.split(",") if p)
        _log_pane_reorder(project_name, project_dir, before, current, now)
    ccm_core.tmux_cmd("set-option", "-w", "-t", win_target,
                      "@ccm_pane_order", ",".join(current))
    return verdict


def pane_reorder_count(now=None, window_sec=86400) -> int:
    """Number of reorder records logged within `window_sec`. Feeds the
    `ccm doctor` row; returns 0 when the log is absent or unreadable."""
    import time as _time
    now = _time.time() if now is None else now
    try:
        with open(pane_order_log_path(), encoding="utf-8") as fh:
    ...
    for line in lines:
        try:
            ...
            continue
        if now - rec.get("ts", 0) <= window_sec:
            count += 1
    return count
